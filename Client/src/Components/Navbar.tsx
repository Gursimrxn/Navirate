  ...
  const handleNavigationRequest = (destination: string) => {
    try {
      ...
      if (destination !== "Emergency") {
        ...
        const found = navItems.find(item => item.name === destination);
        if (found && found.id) {
          navigationEvents.emit(currentLocationId, found.id);
        }
      } else {
      ...
      >
        {isNavigating ? (
          <motion.div className="flex items-center gap-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <span className="font-satoshi text-base font-normal">Continue 12 feet</span>
          </motion.div>
        ) : (
          <div className="flex items-center gap-2">
            {navItems.map((item) => (
              <motion.div
                ...
                onClick={() => handleNavigationRequest(item.name)}
              >
                {item.icon}
                <span className="font-satoshi text-base font-normal">{item.name}</span>
              </motion.div>
            ))}
          </div>
        )}
        ...
        >
          ...
          <path x="0.682617" y="0.697754" width="43.6195" height="43.6195" rx="21.8098" fill="#FFC9C9" />
          <path
